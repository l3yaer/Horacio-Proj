...
static Coordinate project(const Coordinate &coordinate)
{
	float d = M_PI / 180.0f,
		max = 85.0511287798,
...
static Coordinate unproject(const Coordinate &point)
{
	float d = 180.0f / M_PI;
...
static Coordinate transform(const Coordinate &point, float zoom)
{
...
static Coordinate untransform(const Coordinate &point, float zoom)
{
	float scale = scale_zoom(zoom);
...
static Bounds projection_bounds(int zoom)
{
	return { transform(bounds.first, scale_zoom(zoom)), transform(bounds.second, scale_zoom(zoom)) };
}

static Coordinate point_to_coordinate(const Coordinate &point, float zoom)
{
	Coordinate unstransformed_point = untransform(point, zoom);
	return unproject(unstransformed_point);
}

static Coordinate coordinate_to_point(const Coordinate &coordinate, float zoom)
{
