...
MapRenderer::MapRenderer() : FBO(0)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, FRAME_SIZE, FRAME_SIZE, 0,
	...
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, FRAME_SIZE,
			      FRAME_SIZE);
...
void MapRenderer::commit() const
{
	...
	glDisable(GL_BLEND);
	glViewport(0, 0, WindowManager::instance().width,
...
void Map::MapManager::render()
{
	...
	if (dirty)
		map.go_to(World::instance().get_position(), 13);
...
{
	center_tile = factory->get_tile_at(*loader, zoom,
					   World::instance().get_position().y,
					   World::instance().get_position().x);

	position_correction =
		MapCoordinatesAdapter::calculate_position_correction(
			center_tile->coordinate(), zoom,
...
void Map::MapManager::move_camera(int input)
{
	switch (input) {
	case SDLK_UP:
		World::instance().add_position({ 0.0f, 0.00002f });
		break;
	...
	case SDLK_LEFT:
		World::instance().add_position({ -0.00002f, 0.0f });
		break;
