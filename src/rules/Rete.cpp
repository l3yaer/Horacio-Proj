...
Token::Token(BetaMemory *node, WME *wme, Token *parent) : wme(wme), parent(parent), node(node)
{
	if (parent == nullptr)
		token_index = 0;
	else {
...
{
	return first_field_arg == rhs.first_field_arg && second_field_arg == rhs.second_field_arg &&
		   index_second_token_arg == rhs.index_second_token_arg;
}

JoinNode::JoinNode() : alpha_src(nullptr), beta_src(nullptr)
{
}
...
bool Condition::is_satisfied(const WME *wme) const
{
	for (int i = 0; i < WME::Field::COUNT; ++i)
		if (fields[i].type == CField::Type::CONSTANT && fields[i].value != wme->get(i))
			return false;
	return true;
}
...
Net::Net() : dummy(ConstTestNode::dummy())
{
...
void Net::remove(WME *wme)
{
	for (auto item : wme->alpha_memories) {
		item->alpha_memory->items.erase(
				std::remove(item->alpha_memory->items.begin(), item->alpha_memory->items.end(), item),
...
JoinNode *Net::build_join_node(BetaMemory *beta_memory, AlphaMemory *alpha_memory,
...
void Net::lookup_conditions(const std::vector<Condition> &conditions, const std::string value, int *index, int *field)
{
	*index = conditions.size() - 1;
	...
	for (auto condition = conditions.rbegin(); condition != conditions.rend(); ++condition) {
		for (int i = 0; i < WME::Field::COUNT; ++i) {
			if (condition->fields[i].type == CField::Type::VARIABLE && condition->fields[i].value == value) {
				*field = i;
				return;
			}
		}
...
{
	node->alpha_src->successors.erase(
			std::remove(node->alpha_src->successors.begin(), node->alpha_src->successors.end(), node),
		...
		if (const_node->output == node) {
			...
			if (const_node->children.empty()) {
				...
				const_node->parent->children.erase(std::remove(const_node->parent->children.begin(),
															   const_node->parent->children.end(), const_node),
